...
# OpenClaw 自动安全评估脚本
# 根据 OCSS v1.0 标准自动评估系统安全性

echo "==========================================="
echo "OpenClaw 自动安全评估脚本"
echo "根据 OCSS v1.0 标准进行评估"
echo "==========================================="
...
NETWORK_WEIGHTED=$(echo "scale=4; $NETWORK_RATIO * 0.25" | bc -l)
...
# 计算总分
TOTAL_SCORE=$(echo "$NETWORK_WEIGHTED + $AUTH_WEIGHTED + $DATA_WEIGHTED + $LOG_WEIGHTED + $CONFIG_WEIGHTED + $EMERGENCY_WEIGHTED" | bc -l)
# 总分已经是百分比形式 (0-1 范围的数值 * 100)
TOTAL_PERCENTAGE=$(echo "$TOTAL_SCORE * 100" | bc -l)

echo "==========================================="
echo "总体评估结果: $TOTAL_PERCENTAGE%"
echo "安全等级: "
if (( $(echo "$TOTAL_PERCENTAGE >= 90" | bc -l) )); then
    echo "   A+ (极高安全性，符合企业级安全标准)"
elif (( $(echo "$TOTAL_PERCENTAGE >= 80" | bc -l) )); then
    echo "   A (高安全性，适用于大多数商业环境)"
elif (( $(echo "$TOTAL_PERCENTAGE >= 70" | bc -l) )); then
    echo "   B+ (中上安全性，适用于个人和小型团队)"
elif (( $(echo "$TOTAL_PERCENTAGE >= 60" | bc -l) )); then
    echo "   B (中等安全性，需要改进)"
elif (( $(echo "$TOTAL_PERCENTAGE >= 50" | bc -l) )); then
    echo "   C (较低安全性，存在明显风险)"
elif (( $(echo "$TOTAL_PERCENTAGE >= 40" | bc -l) )); then
    echo "   D (低安全性，不推荐生产使用)"
else
    echo "   F (<40分，极低安全性，存在严重风险)"
fi
echo "==========================================="
